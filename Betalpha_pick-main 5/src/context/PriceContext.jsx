...
export const PriceProvider = ({ children }) => {
    ...
    // Calculate current holdings to filter out sold assets
    const holdingsMap = transactions.reduce((acc, tx) => {
        const symbol = tx.asset.toUpperCase();
        const amount = parseFloat(tx.amount);
        if (!acc[symbol]) acc[symbol] = 0;

        if (tx.type === 'buy') acc[symbol] += amount;
        else if (tx.type === 'sell') acc[symbol] -= amount;

        return acc;
    }, {});

    const userTickers = Object.keys(holdingsMap).filter(symbol => holdingsMap[symbol] > 0.000001); // Filter dust
