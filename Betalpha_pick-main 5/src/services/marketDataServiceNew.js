...
export async function fetchOHLC(symbol, interval = '1d', network = 'eth', targetPrice = null) {
    if (!symbol || typeof symbol !== 'string') {
        console.error('[MarketData] Invalid symbol provided to fetchOHLC:', symbol);
        throw new Error('Invalid symbol provided for market data fetch');
    }
...
async function fetchGeckoTerminalOHLC(symbol, interval, network, targetPrice = null) {
    ...
    const ohlcRaw = ohlcData.data?.attributes?.ohlcv_list || [];
    // GeckoTerminal returns newest first (descending), but we need ascending for charts
    const ohlcArray = ohlcRaw.sort((a, b) => a[0] - b[0]);
