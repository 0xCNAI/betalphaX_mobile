...
export const saveAIReport = async (userId, reportData) => {
    ...
    try {
        ...
        // 1. Add to history
        const docRef = await addDoc(reportRef, dataToSave);

        // 2. Update 'latest' pointer (store originalDocId for reference)
        const latestRef = doc(db, 'users', userId, COLLECTION_NAME, 'latest');
        await setDoc(latestRef, { ...dataToSave, updatedAt: timestamp, originalDocId: docRef.id });
