...
export async function analyzeTechnicals(symbol, currentPrice = null, forceRefresh = false) {
    // Bump version to v11 to clear corrupt cache (missing insights) and ensure contract-based fetching
    const cacheKey = `ta_diagnosis_v12_${symbol.toUpperCase()}`;
    ...
    try {
        ...
        const synthesis = synthesizeVerdict(pvAnalysis, momAnalysis, macdAnalysis, structAnalysis, levelAnalysis, bb4h);
...
function synthesizeVerdict(pv, mom, macd, struct, levels, bb4h) {
    let score = 50;
    const signals = [];
    ...
    return {
        ...
        verdicts: {
            short: mom.shortTermVerdict,
            long: levels.longTermVerdict
        },
        levels: {
            shortTerm: {
            ...
            longTerm: {
                support: levels.support * 0.9, // Estimate wider range for long term
                resistance: levels.resistance * 1.1
            }
        },
        volatility_comment: analyzeVolatility(bb4h).isSqueeze ? "Bollinger Bands squeeze - expect volatility." : "Moderate volatility detected."
    };
}
