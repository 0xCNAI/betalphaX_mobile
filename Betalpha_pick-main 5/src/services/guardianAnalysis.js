...
export async function analyzeMaximumRisk(symbol, transactions, currentPrice) {
    try {
        // Use central PnL calculator for consistent average cost
        const { avgBuyPrice: avgHoldingCost, holdings } = calculateAssetPnL(transactions, currentPrice);

        if (holdings <= 0) {
            return {
                hasRisk: false,
                message: "No current position found for risk analysis."
            };
        }
 ...
 */
export async function analyzeProfitTarget(symbol, transactions, currentPrice) {
    try {
        // Use central PnL calculator for consistent average cost
        const { avgBuyPrice, holdings } = calculateAssetPnL(transactions, currentPrice);

        if (holdings <= 0) {
            return {
                hasTarget: false,
                message: "No current position for profit analysis."
            };
        }

        // Find successful trades (buy-sell pairs with profit) used for ratio calculation only
        ...
        if (sellTransactions.length > 0) {
            ...
            sellTransactions.forEach(sellTx => {
                // Find corresponding buy transactions (simplified - assumes FIFO for history analysis)
