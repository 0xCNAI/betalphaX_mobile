...
export async function fetchPrices(tickers) {
    try {
        ...
        if (now - apiCache.prices.timestamp < CACHE_TTL.PRICES) {
            ...
            const allCached = tickers.every(t => t && apiCache.prices.data[String(t).toUpperCase()]);
            if (allCached) {
...
export async function getCoinMetadata(ticker) {
    if (!ticker) return { twitterHandle: null };
    const upperTicker = typeof ticker === 'string' ? ticker.toUpperCase() : String(ticker).toUpperCase(); // Safer usage
    ...
    try {
        ...
        const image = data.image?.large || data.image?.small || null;
        const links = data.links || {}; // Extract all links for Safety Filter

        const result = { twitterHandle, name, image, links };

        // Cache result (permanent cache as handles/images rarely change)
        localStorage.setItem(CACHE_KEY, JSON.stringify(result));

        return result;
    } catch (error) {
        console.error('Error fetching coin metadata:', error);
        return { twitterHandle: null, links: {} };
    }
}
