...
import { searchCoins, validateTicker } from '../services/coinGeckoApi';
import { generateSellSignals, analyzeSellTechnicals } from '../services/technicalAnalysis';
...
// getPrice is provided by usePrices hook, not imported from priceService
import { runPreTradeReview, savePreTradeReviewToSummary } from '../services/aiCoachService'; // NEW IMPORT
...
import { getImportantEvents, getTokenFundamentals } from '../services/fundamentalService';
import { previewAiAnalysis } from '../services/aiAnalysisService';
import ImportantEvents from './ImportantEvents';
import FundamentalWidget from './FundamentalWidget';
import TADiagnosis from './TADiagnosis';
...
const TransactionForm = ({ onClose, initialData = null, initialStep = 1, initialType = null }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    time: initialData?.date ? new Date(initialData.date).toTimeString().slice(0, 5) : new Date().toTimeString().slice(0, 5),
    ...
    coinName: initialData?.coinName || null,
    outcomeStatus: initialData?.outcomeStatus || null,
    ...(initialData || {})
  });
  ...
  const [preTradeReview, setPreTradeReview] = useState(null); // NEW: Store AI Coach Review
  const [translatedReview, setTranslatedReview] = useState(null);
  const [isanalyzingReview, setIsAnalyzingReview] = useState(false); // NEW: Loading state
  const [isAnalyzing, setIsAnalyzing] = useState(false);

  // Step 4 AI Preview State
  const [aiPreview, setAiPreview] = useState(null);
  const [importantEvents, setImportantEvents] = useState([]);
  const [fundamentalData, setFundamentalData] = useState(null);
  const [includeFundamental, setIncludeFundamental] = useState(false);
  ...
  const [proTaData, setProTaData] = useState(null); // Store Pro TA from Step 1/2
  const widgetDataLoadedRef = useRef(false); // Track if we've analyzed with widget data

  // Note Generation State
  const [saveTA, setSaveTA] = useState(false);
  const [saveFundamental, setSaveFundamental] = useState(false);
  const [selectAllEvents, setSelectAllEvents] = useState(false);
  const [selectedEventsData, setSelectedEventsData] = useState([]);



  // Step 4 Accordion State
  // Step 4 Accordion State
  const [expandedSection, setExpandedSection] = useState({
    proTa: false, // Default to collapsed
    fundamental: false, // Default to collapsed
    events: false // Default to collapsed
  });

  const [analysisTriggered, setAnalysisTriggered] = useState({
    coach: false,
    proTa: false,
    fundamental: false,
    events: false
  ...
  const toggleSection = (section) => {
    // Just toggle visibility, do NOT trigger analysis automatically
  ...
  const triggerAnalysis = (section, e) => {
    e.stopPropagation(); // Prevent toggling accordion if clicking the play button (optional, but good UX)
    setAnalysisTriggered(prev => ({ ...prev, [section]: true }));

    // Auto-expand when triggering analysis
    if (!expandedSection[section]) {
      setExpandedSection(prev => ({ ...prev, [section]: true }));
    }
  };
  ...
  // Helper map for translating tags
  const getTagTranslationKey = (tag) => {
    // Remove spaces and special chars to match keys like 'tag_LongTermHold'
    // E.g. "Long Term Hold" -> "LongTermHold"
    // "Staking/Yield" -> "StakingYield"
    const normalized = tag.replace(/[\s\/\(\)]/g, '');
    return `tag_${normalized}`;
  };

  const getExitTagTranslationKey = (tag) => {
    const normalized = tag.replace(/[\s\/\(\)]/g, '');
    return `exit_${normalized}`;
  };

  // Helper to translate a tag string
  const translateTag = (tag) => {
    const key = getTagTranslationKey(tag);
    // If translation exists, return it, else return original tag
    // We check if t(key) returns the key itself (meaning missing translation)
    const translated = t(key);
    return translated !== key ? translated : tag;
  };

  const translateExitTag = (tag) => {
    const key = getExitTagTranslationKey(tag);
    const translated = t(key);
    return translated !== key ? translated : tag;
  };
  ...
  */
    ...
    return (
      <div className="space-y-6">
        <div className="step-header">
          <h4>{t('step2_1') || 'Step 2: Tag Selection'}</h4>
          <p>{t('fundamentalReasons') || 'Categorize your transaction with tags.'}</p>
        </div>
        ...
        <div className="form-group">
          <div className="search-wrapper" style={{ position: 'relative', display: 'flex', gap: '8px' }}>
            ...
            <input
              type="text"
              placeholder={t('selectFundamental') || "Search or create new tag..."}
              value={tagSearch}
        ...
        {savedThesisTags.length > 0 && (
          <div className="tags-section" style={{ marginBottom: '1.5rem' }}>
            <h5 style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '6px' }}>
              <Target size={14} color="var(--accent)" /> {t('buyThesis') || 'Saved Buy Thesis'}
            </h5>
        ...
        <div className="tags-section" style={{ marginBottom: '1.5rem' }}>
          <h5 style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '6px' }}>
            <Sparkles size={14} color="var(--accent)" /> {t('aiInsights') || 'AI Suggested Tags'}
          </h5>
        ...
        <div className="step-actions">
          ...
          <button type="button" onClick={() => setStep(3)} className="btn-primary">
            {t('next')}: {t('sellSignals') || 'Sell Signals'} <ArrowRight size={18} />
          </button>
        </div>
      </div>
    );
  ...
  const generateBuyReasons = async () => {
    ...
    try {
      ...
      setProTaData(technicalReasons); // Store for Step 4
  ...
  const handleTickerSubmit = async (typeOverride = null, shouldAdvance = false) => {
    ...
    try {
      ...
      // FIX: Ensure coinId is set if user manually typed ticker
      if (!formData.coinId && formData.asset) {
        try {
          console.log("Attempting to resolve coinId for:", formData.asset);
          const validation = await validateTicker(formData.asset);
          if (validation && validation.valid && validation.id) {
            console.log("Resolved coinId:", validation.id);
            setFormData(prev => ({
              ...prev,
              coinId: validation.id,
              coinName: validation.name || prev.coinName
            }));
          }
        } catch (err) {
          console.error("Failed to resolve coinId:", err);
        }
      }
  ...
  useEffect(() => {
    if (step === 4 && formData.asset) {
      // Generate Portfolio Overview if not available
      if (!overview) {
        const generateOverview = async () => {
          ...
          if (cached) {
            ...
            const ov = await generatePortfolioOverview(transactions);
            setOverview(ov);
          }
        };
        generateOverview();
      ...
      // Generate Trade Diagnosis if not available
      if (!diagnosis) {
        const runDiagnosis = async () => {
          const diag = await getTradeDiagnosis(transactions);
          setDiagnosis(diag);
        };
        runDiagnosis();
      }
  ...
  }, [step, formData.asset, transactions, diagnosis, overview]);

  // Clear AI advice only when the asset changes (to prevent cross-asset pollution)
  useEffect(() => {
    setPreTradeReview(null); // Clear AI Coach review
    setTranslatedReview(null);
    setIsAnalyzingReview(false);
    widgetDataLoadedRef.current = false; // Reset widget tracking on asset change
  }, [formData.asset, formData.type]);




  // Translation Effect for Pre-Trade Review
  useEffect(() => {
    const translateReview = async () => {
      if (!preTradeReview || language !== 'zh-TW') {
        setTranslatedReview(null);
        return;
      }

      const newReview = { ...preTradeReview };
      if (newReview.behavior_summary) {
        newReview.behavior_summary = await translateText(newReview.behavior_summary, 'zh-TW');
      }

      if (newReview.recommended_playbook) {
        newReview.recommended_playbook = await Promise.all(newReview.recommended_playbook.map(async (rule) => {
          if (typeof rule === 'string') {
            return await translateText(rule, 'zh-TW');
          }
          // Handle objects best effort
          if (typeof rule === 'object' && rule !== null) {
            const newRule = { ...rule };
            if (newRule.rule) newRule.rule = await translateText(newRule.rule, 'zh-TW');
            if (newRule.reasoning) newRule.reasoning = await translateText(newRule.reasoning, 'zh-TW');
            return newRule;
          }
          return rule;
        }));
      }
      setTranslatedReview(newReview);
    };
    translateReview();
  }, [preTradeReview, language]);

  // Auto-fetch price when asset changes
  useEffect(() => {
    // Prevent overwriting price if editing an existing transaction and asset hasn't changed
    if (initialData && initialData.asset === formData.asset && initialData.price) {
      return;
    }
    ...
    return (
      <div className="space-y-6" style={{
        ...
        <div className="step-header" style={{ marginBottom: 0 }}>
          <h4>{t('transactionDetails') || 'Transaction Details'}</h4>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--spacing-md)' }}>
          <div className="form-group" style={{ position: 'relative', marginBottom: 0 }}>
            ...
            <div ref={inputRef} style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <input
                ...
                autoFocus={!initialData?.id && !initialData?.asset}
                disabled={!!initialData?.id || !!initialData?.asset}
                style={{
                  width: '100%',
                  paddingLeft: formData.asset && getIcon && getIcon(formData.asset) ? '40px' : '12px',
                  opacity: (initialData?.id || initialData?.asset) ? 0.7 : 1,
                  cursor: (initialData?.id || initialData?.asset) ? 'not-allowed' : 'text'
                }}
              />
              {/* Autocomplete Dropdown */}
              {showDropdown && !initialData?.id && !initialData?.asset && createPortal(
          ...
          <div className="form-group" style={{ marginBottom: 0 }}>
            <label className="block text-sm font-medium text-slate-400 mb-1">{t('dateAndTime') || 'Date & Time'}</label>
            <div style={{ display: 'flex', gap: '8px' }}>
              <input
                ...
                className="form-input"
                required
                style={{ flex: 1.5, padding: 'var(--spacing-md)' }}
              />
              <input
                type="time"
                name="time"
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--spacing-md)', marginTop: 'var(--spacing-xs)' }}>
          ...
          <div className="form-group" style={{ marginBottom: 0 }}>
            <label className="block text-sm font-medium text-slate-400 mb-1">{t('pricePerCoin') || 'Price per Coin ($)'}</label>
            <input
              type="number"
              name="price"
        ...
        <div className="total-cost-display" style={{
          ...
          <span style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>{t('totalCost') || 'Total Cost'}:</span>
        ...
        <div className="flex flex-col gap-3 mt-2">
          ...
          <div className="flex gap-3">
            ...
            >
              {isOverSelling ? (t('insufficientHoldings') || 'Insufficient Holdings') : (t('sell') || 'Sell')}
            </button>
          </div>
          ...
          >
            <FastForward size={20} /> {t('quickAdd') || 'Quick Add (Skip Analysis)'}
          </button>
        </div>
        ...
        <div className="form-group" style={{ marginBottom: 0, marginTop: 'var(--spacing-lg)' }}>
          <div className="flex justify-between items-center mb-1">
            <div className="flex items-center gap-2">
              <label className="mb-0 text-lg font-bold text-white">{t('investmentNote') || 'Investment Note'}</label>
        ...
        {formData.type === 'buy' && (
          <div className="tags-section-merged mt-4">
            ...
            {aiTags.length > 0 && (
              <div className="mb-3">
                <h5 className="text-xs text-indigo-400 mb-2 flex items-center gap-1"><Sparkles size={12} /> {t('aiSuggested') || 'AI Suggested'}</h5>
            ...
            <div className="mt-10 mb-6">
              <div className="mb-2">
                <h5 className="text-lg font-bold text-white mb-0">{t('recommendedTags') || 'Recommended Tags'}</h5>
              </div>
              <div className="flex flex-wrap gap-2">
                {displayedDefaultTags.map(tag => (
                  <button
  ...
  const handleFundamentalAnalysisComplete = useCallback((data) => {
    // Only update if data actually changed to avoid unnecessary re-renders
    setWidgetFundamentalData(prev => {
      if (JSON.stringify(prev) === JSON.stringify(data.fundamental)) return prev;
      return data.fundamental;
    });
  }, []);

  const handleEventsDataLoaded = useCallback((data) => {
    setWidgetEventsData(prev => {
      if (JSON.stringify(prev) === JSON.stringify(data)) return prev;
      return data;
    });
  }, []);

  const handleRunPreTradeReview = async () => {
    setIsAnalyzingReview(true);
    setPreTradeReview(null); // Clear previous review
    try {
      if (!user) return;
      // Pass userId, assetSymbol, and the draft transaction data
      const review = await runPreTradeReview(user.uid, formData.asset, formData, transactions);
      setPreTradeReview(review);
    } catch (error) {
      console.error("Error running pre-trade review:", error);
  ...
  const renderStep4 = () => {
    ...
    const currentHoldings = transactions
      .filter(tx => tx.asset === formData.asset && tx.type === 'buy' && tx.status === 'open')
      .reduce((acc, tx) => {
    ...
    const displayReview = (language === 'zh-TW' && translatedReview) ? translatedReview : preTradeReview;
    ...
    return (
      <div className="step-4-container">
        ...
        <div className="step-4-sticky-header">
          <div className="step-header" style={{ marginBottom: 0 }}>
            <h4>{t('step4') || 'Step 4: Transaction Details'}</h4>
            <p>{t('transactionDetails') || 'Review transaction details and AI insights before saving.'}</p>
          </div>
        </div>

        {/* Scrollable Body */}
        <div className="step-4-scroll-content">
          <div className="p-4 space-y-4">

            {/* 0. AI COACH REVIEW SECTION (Restored & Upgraded) */}
              ...
              <div className="relative z-10">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="text-xs font-bold text-indigo-400 uppercase tracking-wider flex items-center gap-2">
                    <Sparkles size={16} /> AI Coach Review
                  </h3>
                </div>

                <div className="space-y-3">
                  {!displayReview && !isanalyzingReview && (
                    <div className="text-center py-4">
                      <p className="text-xs text-indigo-200 mb-3">
                        Check how this trade fits your system rules before saving.
                      </p>
                      <button
                        onClick={handleRunPreTradeReview}
                        className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-sm font-medium transition-colors flex items-center gap-2 mx-auto"
                      >
                        <Sparkles size={16} /> Review This Trade Setup
                      </button>
                    </div>
                  )}

                  {isanalyzingReview && (
                    <div className="flex items-center justify-center gap-2 text-indigo-300 text-sm py-4">
                      <Loader2 size={16} className="animate-spin" /> Analyzing your trade against system rules...
                    </div>
                  )}
                    ...
                    <>
                      ...
                      <div className="bg-slate-900/50 rounded-lg p-3 border border-indigo-500/30">
                        <h4 className="text-[10px] uppercase font-bold text-indigo-300 mb-1 flex items-center gap-1">
                          <Activity size={12} /> Coach's Verdict
                        </h4>
                        <p className="text-sm text-indigo-100 leading-relaxed font-medium">
                          "{displayReview.behavior_summary}"
                        </p>
                      </div>

                      {/* Playbook / Checklist */}
                      <div className="space-y-2">
                        <h4 className="text-[10px] uppercase font-bold text-emerald-400 pl-1">Recommended Adjustments</h4>
                        {displayReview.recommended_playbook?.map((rule, idx) => {
                          // Resilient rendering for string or object
                          let ruleText = '';
                          let ruleComment = '';
                          ...
                          if (typeof rule === 'string') {
                            ...
                            // Handle structured commands (e.g. {action: "UPDATE", details: {value: "...", comment: "..."}})
                            if (rule.details && rule.details.value) {
                              ruleText = rule.details.value;
                              ruleComment = rule.details.comment;
                            } else if (rule.details && rule.details.condition) {
                              ruleText = `${rule.details.asset}: ${rule.details.condition}`;
                              ruleComment = rule.details.comment;
                            } else if (rule.rule) {
                              // NEW Strict Schema: { rule, reasoning }
                              ruleText = rule.rule;
                              ruleComment = rule.reasoning;
                            } else {
                              // Fallback for simple objects
                              ruleText = rule.title || rule.description || rule.label || JSON.stringify(rule);
                            }
                          }

                          return (
                            <div key={idx} className="flex flex-col gap-1 bg-emerald-500/10 p-2.5 rounded border border-emerald-500/10 hover:bg-emerald-500/15 transition-colors">
                              <div className="flex items-start gap-2">
                                <CheckSquare size={14} className="text-emerald-400 mt-1 flex-shrink-0" />
                                <span className="text-xs text-emerald-100 font-medium leading-snug">{ruleText}</span>
                              </div>
                              {ruleComment && (
                                <p className="text-[10px] text-emerald-400/70 pl-6 italic border-l-2 border-emerald-500/20 ml-1">
                                  "{ruleComment}"
                                </p>
                              )}
                            </div>
            ...
            {/* 1. Pro Technical Analysis (Existing) */}
            <div className="left-col">
              ...
              <div className="review-card-new">
                <div className="review-card-header">
                  ...
                  <span className={`px-2 py-0.5 rounded text-xs font-bold uppercase ${formData.type === 'buy' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'
                    }`}>
                    {formData.type}
                  </span>
                </div>
                <div className="review-card-body">
                  <div className="detail-row">
                    ...
                    <span className="detail-value">{formData.asset}</span>
                  </div>
                  <div className="detail-row">
                  ...
                  <div className="detail-row">
                    ...
                    <span className="detail-value">${(parseFloat(formData.amount || 0) * parseFloat(formData.price || 0)).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="review-card-new">
                ...
                <div className="review-card-body space-y-4">
                  {/* Buy Thesis */}
                  <div>
                    <h6 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2 flex items-center gap-1">
                      <Target size={12} /> Buy Thesis
                    </h6>
                    <div className="flex flex-wrap gap-2">
                      {formData.tags?.map(tag => (
                        <span key={tag} className="px-2.5 py-1 rounded-full text-xs font-medium bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                          {tag}
                        </span>
                      ))}
                      {(!formData.tags || formData.tags.length === 0) && <span className="text-gray-500 text-sm italic">No thesis tags selected</span>}
                    </div>
                  </div>

                  {/* Exit Strategy */}
                  {(formData.exitTags?.length > 0 || formData.exitStrategy) && (
                    <div>
                      <h6 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2 flex items-center gap-1">
                        <TrendingDown size={12} /> Exit Strategy
                      </h6>
                      <div className="flex flex-wrap gap-2">
                        {formData.exitTags?.map(tag => (
                          <span key={tag} className="px-2.5 py-1 rounded-full text-xs font-medium bg-indigo-500/10 text-indigo-400 border border-indigo-500/20">
                            {tag}
                          </span>
                        ))}
                      </div>
                  ...
                  {/* Investment Note */}
                  {formData.investmentNotes?.some(note => note) && (
                    <div>
                      <h6 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2 flex items-center gap-1">
                        <FileText size={12} /> Investment Note
                      </h6>
                      <div className="space-y-2 max-h-[200px] overflow-y-auto pr-1 custom-scrollbar">
                        {formData.investmentNotes?.map((note, i) => (
                          note && <p key={i} className="text-sm text-gray-300 leading-relaxed bg-white/5 p-3 rounded border border-white/5">
                            {note}
                          </p>
                        ))}
                      </div>
            ...
            {/* Right Column: AI Insights (Accordions) */}
            <div className="right-col">
              ...
              <div className="flex gap-3 items-start">
                {/* Sidebar Play Button */}
                <button
                  className={`flex-shrink-0 mt-1 p-2 rounded-lg transition-all ${analysisTriggered.proTa
                    ? 'bg-blue-500/20 text-blue-400 cursor-default'
                    : 'bg-slate-800 hover:bg-blue-600 hover:text-white text-slate-400'
                    }`}
                  onClick={(e) => !analysisTriggered.proTa && triggerAnalysis('proTa', e)}
                  title={analysisTriggered.proTa ? "Analysis Active" : "Run Pro Analysis"}
                  disabled={analysisTriggered.proTa}
                >
                  {analysisTriggered.proTa ? (
                    <Activity size={18} />
                  ) : (
                ...
                {/* Accordion Container */}
                <div className="flex-grow accordion-item">
                  <div className="accordion-header" onClick={() => toggleSection('proTa')}>
                    <div className="flex items-center gap-2 cursor-pointer">
                      <span className="font-medium text-slate-300">Pro Technical Analysis</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <button
                        onClick={(e) => { e.stopPropagation(); setSaveTA(!saveTA); }}
                        className={`transition-colors ${saveTA ? 'text-indigo-400' : 'text-gray-600 hover:text-gray-400'}`}
                        title={saveTA ? "Don't add to note" : "Add to note"}
                      >
                        {saveTA ? <CheckSquare size={18} /> : <Square size={18} />}
                      </button>
                      <div className="cursor-pointer">
                        {expandedSection.proTa ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                      </div>
                    </div>
                  </div>

                  <div className="accordion-content" style={{ display: expandedSection.proTa ? 'block' : 'none' }}>
                    {analysisTriggered.proTa ? (
                      <TADiagnosis
                      ...
                      <div className="p-6 text-center text-gray-500 text-sm bg-slate-900/50 rounded-lg border border-slate-800 border-dashed">
                        <p className="mb-2">Analysis not started.</p>
                        <button
                          onClick={(e) => triggerAnalysis('proTa', e)}
                          className="text-indigo-400 hover:text-indigo-300 underline"
                        >
                          Click Play button to run analysis
                        </button>
                      </div>
                    )}
                  </div>
              ...
              <div className="flex gap-3 items-start mt-4">
                <button
                  className={`flex-shrink-0 mt-1 p-2 rounded-lg transition-all ${analysisTriggered.fundamental
                    ? 'bg-purple-500/20 text-purple-400 cursor-default'
                    : 'bg-slate-800 hover:bg-purple-600 hover:text-white text-slate-400'
                    }`}
                  onClick={(e) => !analysisTriggered.fundamental && triggerAnalysis('fundamental', e)}
                  title={analysisTriggered.fundamental ? "Analysis Active" : "Run Fundamental Analysis"}
                  disabled={analysisTriggered.fundamental}
                >
                  {analysisTriggered.fundamental ? (
                    <Target size={18} />
                  ) : (
                    <Play size={18} fill="currentColor" />
                  )}
                ...
                <div className="flex-grow accordion-item">
                  <div className="accordion-header" onClick={() => toggleSection('fundamental')}>
                    <div className="flex items-center gap-2 cursor-pointer">
                      <span className="font-medium text-slate-300">Fundamental Intelligence</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <button
                        onClick={(e) => { e.stopPropagation(); setSaveFundamental(!saveFundamental); }}
                        className={`transition-colors ${saveFundamental ? 'text-indigo-400' : 'text-gray-600 hover:text-gray-400'}`}
                        title={saveFundamental ? "Don't add to note" : "Add to note"}
                      >
                        {saveFundamental ? <CheckSquare size={18} /> : <Square size={18} />}
                      </button>
                      <div className="cursor-pointer">
                        {expandedSection.fundamental ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                      </div>
                    </div>
                  </div>

                  <div className="accordion-content" style={{ display: expandedSection.fundamental ? 'block' : 'none' }}>
                    {analysisTriggered.fundamental ? (
                      <FundamentalWidget
                        symbol={formData.asset}
                        embedded={true}
                        onAnalysisComplete={handleFundamentalAnalysisComplete}
                      />
                    ) : (
                      <div className="p-6 text-center text-gray-500 text-sm bg-slate-900/50 rounded-lg border border-slate-800 border-dashed">
                        <p className="mb-2">Analysis not started.</p>
                        <button
                          onClick={(e) => triggerAnalysis('fundamental', e)}
                          className="text-indigo-400 hover:text-indigo-300 underline"
                        >
                          Click Play button to run analysis
                        </button>
              ...
              <div className="flex gap-3 items-start mt-4">
                <button
                  className={`flex-shrink-0 mt-1 p-2 rounded-lg transition-all ${analysisTriggered.events
                    ? 'bg-orange-500/20 text-orange-400 cursor-default'
                    : 'bg-slate-800 hover:bg-orange-600 hover:text-white text-slate-400'
                    }`}
                  onClick={(e) => !analysisTriggered.events && triggerAnalysis('events', e)}
                  title={analysisTriggered.events ? "Analysis Active" : "Run Event Analysis"}
                  disabled={analysisTriggered.events}
                >
                  {analysisTriggered.events ? (
                    <Newspaper size={18} />
                  ) : (
                    <Play size={18} fill="currentColor" />
                  )}
                ...
                <div className="flex-grow accordion-item">
                  <div className="accordion-header" onClick={() => toggleSection('events')}>
                    <div className="flex items-center gap-2 cursor-pointer">
                      <span className="font-medium text-slate-300">Important Events & Insights</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <button
                        onClick={(e) => { e.stopPropagation(); setSelectAllEvents(!selectAllEvents); }}
                        className={`transition-colors ${selectAllEvents ? 'text-indigo-400' : 'text-gray-600 hover:text-gray-400'}`}
                        title={selectAllEvents ? "Deselect All Events" : "Select All Events"}
                      >
                        {selectAllEvents ? <CheckSquare size={18} /> : <Square size={18} />}
                      </button>
                      <div className="cursor-pointer">
                  ...
                  <div className="accordion-content" style={{ display: expandedSection.events ? 'block' : 'none' }}>
                    {analysisTriggered.events ? (
                      <ImportantEvents
                        symbol={formData.asset}
                        embedded={true}
                        selectable={true}
                        externalSelectAll={selectAllEvents}
                        onSelectionChange={setSelectedEventsData}
                        onDataLoaded={handleEventsDataLoaded}
                      />
                    ) : (
                      <div className="p-6 text-center text-gray-500 text-sm bg-slate-900/50 rounded-lg border border-slate-800 border-dashed">
                        <p className="mb-2">Analysis not started.</p>
                        <button
                          onClick={(e) => triggerAnalysis('events', e)}
                          className="text-indigo-400 hover:text-indigo-300 underline"
                        >
                          Click Play button to run analysis
                        </button>
        ...
        {/* Sticky Footer */}
        <div className="step-4-sticky-footer">
          <div className="step-actions">
            <button
              type="button"
              onClick={() => setStep(3)}
            ...
            <button
              ...
              className={`btn-primary ${isOverSelling ? 'opacity-50 cursor-not-allowed' : ''}`}
              disabled={isOverSelling}
            >
        ...
        {isOverSelling && (
          <div className="absolute bottom-20 left-0 right-0 text-red-400 text-xs text-center z-50 pointer-events-none">
            <span className="bg-slate-900/80 px-3 py-1 rounded-full border border-red-500/30 backdrop-blur">
              Warning: You are selling more than you hold ({effectiveHoldings} available).
            </span>
          </div>
        )}
      </div>
  ...
  const renderSellStep2 = () => {
    ...
    return (
      <div className="space-y-6">
        <div className="step-header">
          <h4>{t('step2_linkNarrative') || 'Step 2: Link Narrative'}</h4>
          <p>{t('whatOriginalThesis') || 'What original thesis are you closing? (Sorted by recent)'}</p>
        </div>
        ...
        {uniqueReasons.length > 0 ? (
          ...
          <div className="no-positions-warning">
            ...
            <p>{t('noActiveNarratives') || `No active buy narratives found for ${formData.asset}.`}</p>
          </div>
        )}
        ...
        <div className="step-actions">
          ...
          >
            {t('next')}: {t('outcome') || 'Outcome'} <ArrowRight size={18} />
          </button>
        </div>
      </div>
    );
  ...
  const renderSellStep3 = () => {
    ...
    return (
      <div className="space-y-6">
        <div className="step-header">
          <h4>{t('outcomeAndFactors') || 'Outcome & Factors'}</h4>
          <p>{t('classifyResult') || 'Classify the result and document the cause.'}</p>
        </div>

        {/* 1. Outcome Status (Tags) */}
        <div className="form-group">
          <label>{t('outcomeStatus') || 'Outcome Status'}</label>
          <div className="tags-container">
            ...
            <div className="custom-tag-input" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px' }}>
              <input
                type="text"
                placeholder={t('newOutcome') || "+ New Outcome"}
              ...
              {formData.customOutcome && (
                <button type="button" onClick={() => handleAddCustomTag('outcome')} className="btn-secondary small"><Plus size={14} /></button>
              )}
            </div>
          </div>
        </div>
          ...
          <div className="tags-container" style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
            {Object.values(exitFactors).flat().map(factor => (
              <button
                key={factor}
                type="button"
                className={`tag-pill ${formData.exitFactors.includes(factor) ? 'active' : ''}`}
                onClick={() => {
                  setFormData(prev => {
                ...
                style={{
                  padding: '6px 12px',
                  borderRadius: '16px',
                  border: formData.exitFactors.includes(factor) ? '1px solid var(--accent-primary)' : '1px solid var(--bg-tertiary)',
                  backgroundColor: formData.exitFactors.includes(factor) ? 'rgba(99, 102, 241, 0.1)' : 'var(--bg-secondary)',
                  color: formData.exitFactors.includes(factor) ? 'var(--accent-primary)' : 'var(--text-secondary)',
                  cursor: 'pointer',
                  fontSize: '0.85rem',
                  fontWeight: formData.exitFactors.includes(factor) ? '500' : '400',
                  transition: 'all 0.2s'
                }}
              >
                {factor}
            ...
            <div className="custom-tag-input" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px' }}>
              ...
              {formData.customExitFactor && (
                <button type="button" onClick={() => handleAddCustomTag('factor')} className="btn-secondary small"><Plus size={14} /></button>
              )}
            </div>
          </div>
        </div>
  ...
  const renderStep3 = () => {
    ...
    return (
      <div className="space-y-6">
        <div className="step-header">
          <h4>{t('exitStrategy') || 'Exit Strategy'}</h4>
          <p>{t('planExit') || 'Plan your exit. When will you take profit or cut losses?'}</p>
        </div>
        ...
        <div className="reasons-grid">
          ...
          <div className="custom-indicator-builder" style={{
            ...
            <div style={{ flex: 2 }}>
              <label style={{ display: 'block', fontSize: '0.8rem', color: 'var(--text-secondary)', marginBottom: '4px' }}>{t('indicator') || 'Indicator'}</label>
              <select
            ...
            <div style={{ flex: 1 }}>
              <label style={{ display: 'block', fontSize: '0.8rem', color: 'var(--text-secondary)', marginBottom: '4px' }}>{t('value') || 'Value'}</label>
              <input
                type="number"
          ...
          <div className="exit-tag-section" style={{ marginBottom: '1.5rem' }}>
            <h5 style={{ fontSize: '0.95rem', marginBottom: '1rem', color: 'var(--text-primary)' }}>{t('exitStrategyTags') || 'Exit Strategy Tags'}</h5>
            ...
            <div className="form-group">
              <div className="search-wrapper" style={{ position: 'relative', display: 'flex', gap: '8px' }}>
                ...
                <input
                  type="text"
                  placeholder={t('searchExitTag') || "Search or create new exit tag..."}
                  value={exitTagSearch}
                ...
                {isCustomExitTag && (
                  ...
                  >
                    <Plus size={16} /> {t('add')} "{exitTagSearch}"
                  </button>
                )}
              </div>
            </div>
            ...
            {formData.exitTags && formData.exitTags.length > 0 && (
              <div className="selected-tags-area" style={{ marginBottom: '1.5rem' }}>
                <h5 style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', marginBottom: '0.75rem' }}>{t('selectedExitTags') || 'Selected Exit Tags'}</h5>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                  {formData.exitTags.map(tag => (
                    <span
                      key={tag}
            ...
            {aiExitTags.length > 0 && (
              <div className="tags-section" style={{ marginBottom: '1.5rem' }}>
                <h5 style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <Sparkles size={14} color="var(--accent)" /> {t('aiSuggestedExitTags') || 'AI Suggested Exit Tags'}
                </h5>
                <div className="tags-grid" style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                  {aiExitTags.map(tag => (
                    <button
            ...
            <div className="tags-section">
              <h5 style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', marginBottom: '0.5rem' }}>
                {t('recommendedExitTags') || 'Recommended Exit Tags'}
              </h5>
              <div className="tags-grid" style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
  ...
  const generateAutoNotes = () => {
    ...
    if (saveTA && proTaData) {
      ...
      notes += `Score: ${proTaData.score}/100 (${proTaData.action})\n`;
      if (proTaData.insights?.length) {
        notes += `Insights: ${proTaData.insights[0].content}\n`;
      }
    }
    if (saveFundamental && widgetFundamentalData) {
      notes += `\n[Fundamental Intelligence - ${today}]\n`;
      if (widgetFundamentalData.valuation) {
        notes += `MCap: $${(widgetFundamentalData.valuation.mcap / 1e6).toFixed(2)}M | FDV: $${(widgetFundamentalData.valuation.fdv / 1e6).toFixed(2)}M\n`;
      }
      if (widgetFundamentalData.growth) {
        notes += `TVL: $${(widgetFundamentalData.growth.tvl_current / 1e6).toFixed(2)}M\n`;
      }
    }
    if (selectedEventsData.length > 0) {
      ...
      selectedEventsData.forEach(event => {
        notes += `- ${event.title}\n`;
        if (event.detail) notes += `  ${event.detail}\n`;
      });
    }
    return notes;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    let effectiveNotes = [...(formData.investmentNotes || [])];
    if (autoNotes) {
      effectiveNotes.push(autoNotes);
    }
    ...
    try {
      // Combine notes
      const finalMemo = effectiveNotes.filter(n => n && n.trim()).join('\n\n');
      const finalExitMemo = formData.exitNotes.filter(n => n.trim()).join('\n\n');

      // Resolve Tag Links
      // Initialize with existing links (from Add to Thesis)
      const tagLinks = { ...(formData.reasonLinks || {}) };
      ...
      const timePart = formData.time ? (formData.time.length === 5 ? `${formData.time}:00` : formData.time) : '00:00:00';
      const fullDate = new Date(`${datePart}T${timePart}`);
      const isoDate = !isNaN(fullDate.getTime()) ? fullDate.toISOString() : new Date().toISOString();

      const transactionData = {
        ...formData,
        date: isoDate, // Override split date with full ISO timestamp incl. time
        ...
        tagLinks: tagLinks, // Save the links (legacy)
        reasonLinks: tagLinks, // Save the links (new standard)
        timestamp: new Date().toISOString(),
        pnl: 0, // Initial PnL is 0

        // Save AI Data if selected
        ai_fundamental: includeFundamental ? widgetFundamentalData : null,
        ai_events: includeEvents ? widgetEventsData : null,
        ai_technical: proTaData || null, // Always save Pro TA if available
        status: 'open',
        // Unified Market Context Snapshot
        market_context_snapshot: {
          timestamp: Date.now(),
          // Global Market (Placeholders - to be filled by context/service if available)
          btcDominance: null,
          ...
          // Token Level (Placeholders - filled by TransactionContext)
          fdv_ratio: null,
          tvl_trend_30d: null,
          sector_tags: [],

          // Price / TA (Placeholders)
          price: priceVal,
        ...
        ai_exit_plan: null,
        ai_risk_comment: aiPreview?.ai_risk_comment || null,
        ...
        ai_fundamental_insights: (widgetFundamentalData?.analysis && includeFundamental) ? {
          items: [
        ...
        important_events_snapshot: (widgetEventsData && includeEvents) ? {
        ...
        ai_events_insights: null, // Deprecated or Future Use (Widget handles display)
      ...
      if (initialData && initialData.id) {
        ...
        // 1b. If there was an AI Coach Review (via "Review This Trade"), persist it to Asset Summary
        if (preTradeReview) {
          console.log('[TransactionForm] Persisting AI Coach Review to Asset Summary...');
          // No await needed, let it run in background
          savePreTradeReviewToSummary(user.uid, formData.asset, preTradeReview);
        }
      }
      onClose();
    } catch (error) {
...
/* --- New Step 4 Redesign Styles --- */

.step-4-container {
  display: flex;
  flex-direction: column;
  height: 80vh; /* Fixed height */
  min-height: 600px; /* Minimum reasonable height */
  overflow-y: hidden; /* Only the body scroll */
}
...
.step-4-sticky-header {
  position: sticky;
  top: 0;
  background-color: #1a1b1e; /* Match modal bg */
  z-index: 20;
  padding-bottom: 16px;
  border-bottom: 1px solid var(--bg-tertiary);
  margin-bottom: 16px;
