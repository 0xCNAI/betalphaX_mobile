    ...
    async process() {
        ...
        try {
            ...
            if (isRateLimit) {
                ...
                // Exponential backoff for rate limits: 1x, 2x, 4x, 8x (max)
                this.backoffMultiplier = Math.min(Math.pow(2, this.consecutiveErrors - 1), 8);
                console.warn(`[RequestQueue] Rate limit (429) detected. Backoff multiplier: ${this.backoffMultiplier}x`);
                ...
                // For 503, just retry or let the caller handle failover, don't trip circuit breaker immediately
                // But still use a small backoff to be polite
                this.backoffMultiplier = 2;
                console.warn(`[RequestQueue] Service Unavailable (503). Retrying might work.`);
            }
            reject(error);
        } finally {
            this.processing = false;
